...
FHitResult UOWNGameplayAbility_FromRangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, bool bIsSimulated, TArray<FHitResult>& OutHitResults) const
{
	...
	GetWorld()->LineTraceMultiByChannel(HitResults, StartTrace, EndTrace, ECollisionChannel::ECC_EngineTraceChannel2, TraceParams);
...
/*#if ENABLE_DRAW_DEBUG
	if (OWNConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		...
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, OWNConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
	...
	if (FindFirstPawnHitResult(OutHits) == INDEX_NONE)
	{
		...
		Impact = WeaponTrace(StartTrace, EndTrace, bIsSimulated, /*out*/ SweepHits);
...
void UOWNGameplayAbility_FromRangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, TArray<FHitResult>& OutHits)
{
	//UOWNRangedWeaponInstance* WeaponData = InputData.WeaponData;
	//check(WeaponData);

	//Keeping for when we implement shotguns
	...
	//{
		...
		const FVector BulletDir = VRandConeNormalDistribution(InputData.AimDir, 0, 0);

		//hard coded max distance atm will pull from weapon tags at a later date.
		const FVector EndTrace = InputData.StartTrace + (BulletDir * 500);
		...
		{
/*#if ENABLE_DRAW_DEBUG
			if (OWNConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, OWNConsoleVariables::DrawBulletHitRadius, FColor::Red, false, OWNConsoleVariables::DrawBulletHitRadius);
			}
#endif*/
...
void UOWNGameplayAbility_FromRangedWeapon::PerformLocalTargeting(TArray<FHitResult>& OutHits)
{
	...
	if (AvatarPawn && AvatarPawn->IsLocallyControlled()/* && WeaponData*/)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EOWNAbilityTargetingSource::CameraTowardsFocus);
		InputData.AimDir = TargetTransform.GetUnitAxis(EAxis::X);
		InputData.StartTrace = TargetTransform.GetTranslation();

		InputData.EndAim = InputData.StartTrace + InputData.AimDir;

#if ENABLE_DRAW_DEBUG
		//if (OWNConsoleVariables::DrawBulletTracesDuration > 0.0f)
		//{
		//	static float DebugThickness = 2.0f;
		//	DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, OWNConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		//}
#endif
